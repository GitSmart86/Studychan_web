...
class Groupdeck(models.Model):
    ...
    owner = models.ForeignKey(User,
                              related_name='user_groupdeck',
                              on_delete=models.CASCADE)

    format_type = models.ForeignKey(Format,
                                    related_name='format_groupdeck',
    ...
    def upload_media(self, instance):
        newFile = 'user/{0}/icon_groupdeck/{1}'.format(
    ...
    ratings = models.ManyToManyField(User,
                                     related_name='groupdeck_ratings',
                                     blank=True,
                                     symmetrical=False,
                                     through='Junct_Rate_Groupdeck')

    subscribers = models.ManyToManyField(User,
                                         related_name='groupdeck_subscriptions',
                                         blank=True,
                                         symmetrical=False,
                                         through='Junct_Sub_Groupdeck')

    phylum_tags = models.ManyToManyField(Phylum_Tag,
                                         related_name='groupdecks_phylum_tags',
                                         blank=True,
                                         symmetrical=False,
                                         through='Junct_PhylumTag_Groupdeck')
    ...
    #     super(Groupdeck, self).save(*args, **kwargs)

    #     if self.icon is not None:
    #         if "None." in self.icon.name:
    #             savedSelf = Groupdeck.objects.filter(name=self.name).last()
    #             newName = '/user/{0}/icon_groupdeck/{1}.jpg'.format(
